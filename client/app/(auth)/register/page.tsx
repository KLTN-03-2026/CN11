...
export default function LoginForm() {
    ...
    useEffect(() => {
        if (phone.length >= 8 && password.length >= 6 && captchaInput.length >= 2 && username.length >= 4) {
            setCheckRegister(true);
        } else {
            setCheckRegister(false);
        }
    }, [phone, password, captchaInput, username])
    ...
    async function handleRegister(e?: FormEvent) {
        e?.preventDefault();
        if (!remember) {
             messageApi.warning("Bạn vui lòng đồng ý chập nhập điều khoản & bảo mật để đăng ký tài khoản SmartDine.");
            return;
        }
        ...
        try {
            ...
            setTimeout(async () => {
                ...
                const responsive = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/auth/register`, {
                    ...
                    body: JSON.stringify({ phone: phone, password: password, username: username })
                })
            ...
            >
                ...
                <div id="login" className="col-span-12 lg:col-span-5 p-8">
                    <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
                        {<div>
                            <h3 className="text-xl font-semibold">Đăng ký tài khoản SmartDine</h3>
                            <p className="text-sm text-gray-500 mt-1">Nhập tên tài khoản,số điện thoại và mật khẩu để đăng ký.</p>
                            ...
                            <form onSubmit={handleRegister} className="mt-6 space-y-4">
                                ...
                                <motion.div>
                                    <label className="block text-xs font-medium text-gray-700">Tên tài khoản</label>
                                ...
                                <motion.div className="flex items-center justify-between">
                                    <label className="flex items-center justify-center gap-2 text-sm">
                                        ...
                                        <span className="text-gray-600">Tôi đồng ý về điều khoản & bảo mật</span>
                                    </label>
                                </motion.div>
                                ...
                                <div className="flex gap-2">
                                    <Button
                                        type="submit"
                                        className={`flex-1 justify-center ${checkRegister ? "bg-indigo-600 hover:bg-indigo-700 " : "bg-gray-400"} cursor-pointer text-white rounded-md px-4 py-2 font-medium shadow hover:scale-[1.01] active:scale-95`}
                                        disabled={loading}
                                    >
                                ...
                                <div className="flex items-center justify-center">
                                    <label className="block text-xs font-medium text-gray-700">Tôi đã có tài khoản? <Link className="text-indigo-600 hover:underline" href="/login">Đăng nhập</Link></label>
                                </div>
